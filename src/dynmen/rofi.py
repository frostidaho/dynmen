...
class Rofi(TraitMenu):
    ...
    no_config = Flag(
        '-no-config', info_text='Do not load configuration, use default values.'
    )
    version = Flag('-version', info_text='Print the version number and exit.')
    dmenu = Flag('-dmenu', default_value=True)
    display = Option('-display', info_text='X server to contact. @@@ ${DISPLAY}')
    ...
    e = Option('-e', info_text='Show a dialog displaying the passed message and exit.')
    ...
    show = Option(
        '-show', info_text="Show the mode 'mode' and exit. The mode has to be enabled."
    )
    lazy_grab = Flag(
        '-lazy-grab',
    ...
    only_match = Flag('-only-match', info_text='Force selection or custom entry')
    ...
    password = Flag(
        '-password', info_text="Do not show what the user inputs. Show '*' instead."
    )
    markup_rows = Flag(
        '-markup-rows', info_text='Allow and render pango markup as input data.'
    )
    sep = Option('-sep', default_value='\x00')
    input = Option(
        '-input', info_text='Read input from file instead from standard input.'
    )
    sync = Flag(
        '-sync', info_text='Force dmenu to first read all input data, then show dialog.'
    )
    modi = Option('-modi', info_text='Enabled modi @@@ window,run,ssh (Default)')
    ...
    location = Option('-location', info_text='Location on screen @@@ 0 (Default)')
    ...
    fixed_num_lines = Flag(
        '-fixed-num-lines', info_text='Always show number of lines @@@ True (Default)'
    )
    no_fixed_num_lines = Flag(
        '-no-fixed-num-lines', info_text='Always show number of lines @@@ True (Default)'
    )
    terminal = Option(
        '-terminal', info_text='Terminal to use @@@ rofi-sensible-terminal (Default)'
    )
    ssh_client = Option('-ssh-client', info_text='Ssh client to use @@@ ssh (Default)')
    ssh_command = Option(
        '-ssh-command',
    ...
    run_list_command = Option(
        '-run-list-command', info_text='Command to get extra run targets @@@  (Default)'
    )
    ...
    disable_history = Flag(
        '-disable-history', info_text='Disable history in run/ssh @@@ False (Default)'
    )
    no_disable_history = Flag(
        '-no-disable-history', info_text='Disable history in run/ssh @@@ False (Default)'
    )
    levenshtein_sort = Flag(
        '-levenshtein-sort', info_text='Use levenshtein sorting @@@ False (Default)'
    )
    no_levenshtein_sort = Flag(
        '-no-levenshtein-sort', info_text='Use levenshtein sorting @@@ False (Default)'
    )
    case_sensitive = Flag(
        '-case-sensitive', info_text='Set case-sensitivity @@@ False (Default)'
    )
    no_case_sensitive = Flag(
        '-no-case-sensitive', info_text='Set case-sensitivity @@@ False (Default)'
    )
    cycle = Flag('-cycle', info_text='Cycle through the results list @@@ True (Default)')
    no_cycle = Flag(
        '-no-cycle', info_text='Cycle through the results list @@@ True (Default)'
    )
    sidebar_mode = Flag(
        '-sidebar-mode', info_text='Enable sidebar-mode @@@ False (Default)'
    )
    no_sidebar_mode = Flag(
        '-no-sidebar-mode', info_text='Enable sidebar-mode @@@ False (Default)'
    )
    eh = Option('-eh', info_text='Row height (in chars) @@@ 1 (Default)')
    auto_select = Flag(
        '-auto-select', info_text='Enable auto select mode @@@ False (Default)'
    )
    no_auto_select = Flag(
        '-no-auto-select', info_text='Enable auto select mode @@@ False (Default)'
    )
    parse_hosts = Flag(
        '-parse-hosts', info_text='Parse hosts file for ssh mode @@@ False (Default)'
    )
    no_parse_hosts = Flag(
        '-no-parse-hosts', info_text='Parse hosts file for ssh mode @@@ False (Default)'
    )
    ...
    tokenize = Flag('-tokenize', info_text='Tokenize input string @@@ True (Default)')
    no_tokenize = Flag(
        '-no-tokenize', info_text='Tokenize input string @@@ True (Default)'
    )
    m = Option('-m', info_text='Monitor id to show on @@@ -5 (Default)')
    line_margin = Option('-line-margin', info_text='Margin between rows @@@ 2 (Default)')
    line_padding = Option(
        '-line-padding', info_text='Padding within rows @@@ 1 (Default)'
    )
    filter = Option('-filter', info_text='Pre-set filter @@@ (unset) (Default)')
    ...
    fullscreen = Flag('-fullscreen', info_text='Fullscreen @@@ False (Default)')
    no_fullscreen = Flag('-no-fullscreen', info_text='Fullscreen @@@ False (Default)')
    fake_transparency = Flag(
        '-fake-transparency', info_text='Fake transparency @@@ False (Default)'
    )
    no_fake_transparency = Flag(
        '-no-fake-transparency', info_text='Fake transparency @@@ False (Default)'
    )
    dpi = Option('-dpi', info_text='DPI @@@ -1 (Default)')
    threads = Option(
        '-threads', info_text='Threads to use for string matching @@@ 0 (Default)'
    )
    ...
    click_to_exit = Flag(
        '-click-to-exit', info_text='Click outside the window to exit @@@ True (Default)'
    )
    ...
    pid = Option(
        '-pid', info_text='Pidfile location @@@ /run/user/1001/rofi.pid (Default)'
    )
    ...
    kb_secondary_paste = Option(
        '-kb-secondary-paste', info_text='Paste clipboard @@@ Control+v,Insert (Default)'
    )
    kb_clear_line = Option(
        '-kb-clear-line', info_text='Clear input line @@@ Control+w (Default)'
    )
    kb_move_front = Option(
        '-kb-move-front', info_text='Beginning of line @@@ Control+a (Default)'
    )
    kb_move_end = Option('-kb-move-end', info_text='End of line @@@ Control+e (Default)')
    kb_move_word_back = Option(
        '-kb-move-word-back', info_text='Move back one word @@@ Alt+b (Default)'
    )
    kb_move_word_forward = Option(
        '-kb-move-word-forward', info_text='Move forward one word @@@ Alt+f (Default)'
    )
    kb_move_char_back = Option(
        '-kb-move-char-back', info_text='Move back one char @@@ Left,Control+b (Default)'
    )
    ...
    kb_row_down = Option(
        '-kb-row-down', info_text='Select next entry @@@ Down,Control+n (Default)'
    )
    kb_row_tab = Option(
        '-kb-row-tab',
    ...
    kb_page_prev = Option(
        '-kb-page-prev', info_text='Go to the previous page @@@ Page_Up (Default)'
    )
    kb_page_next = Option(
        '-kb-page-next', info_text='Go to the next page @@@ Page_Down (Default)'
    )
    kb_row_first = Option(
        '-kb-row-first', info_text='Go to the first entry @@@ Home,KP_Home (Default)'
    )
    kb_row_last = Option(
        '-kb-row-last', info_text='Go to the last entry @@@ End,KP_End (Default)'
    )
    ...
    kb_custom_11 = Option(
        '-kb-custom-11', info_text='Custom keybinding 11 @@@ Alt+exclam (Default)'
    )
    kb_custom_12 = Option(
        '-kb-custom-12', info_text='Custom keybinding 12 @@@ Alt+at (Default)'
    )
    kb_custom_13 = Option(
        '-kb-custom-13', info_text='Csutom keybinding 13 @@@ Alt+numbersign (Default)'
    )
    kb_custom_14 = Option(
        '-kb-custom-14', info_text='Custom keybinding 14 @@@ Alt+dollar (Default)'
    )
    kb_custom_15 = Option(
        '-kb-custom-15', info_text='Custom keybinding 15 @@@ Alt+percent (Default)'
    )
    kb_custom_16 = Option(
        '-kb-custom-16',
    ...
    kb_custom_17 = Option(
        '-kb-custom-17', info_text='Custom keybinding 17 @@@ Alt+ampersand (Default)'
    )
    kb_custom_18 = Option(
        '-kb-custom-18', info_text='Custom keybinding 18 @@@ Alt+asterisk (Default)'
    )
    kb_custom_19 = Option(
        '-kb-custom-19', info_text='Custom Keybinding 19 @@@ Alt+parenleft (Default)'
    )
    display_ssh = Option(
        '-display-ssh',
